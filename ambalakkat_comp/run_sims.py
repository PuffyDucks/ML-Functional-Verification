...
def ensure_csv(csv_path):
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["a", "b", "match", "passed"]) # <<< FEATURES AND TARGET HEDAERS
...
async def run_simulation_batch(dut, csv_path, input_batch):
    with open(csv_path, 'a', newline='') as f:
        ...
        for a_val, b_val in input_batch: # \/ \/ \/ SIMULATION INPUTS AND OUTPUTS \/ \/ \/
            ...
            passed = int(match == (a_val * b_val if a_val == b_val else 0))

            writer.writerow([a_val, b_val, match, passed])
